...
ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs)
{
	...
	while (offset_begin + offset < offset_end) {
        ...
        if (syscall_read_dev((u_int)&diskno, dev_addr + 0x10, 4) < 0)
            user_panic("ide_read err 0x10\n");
        if (syscall_read_dev((u_int)&tmp_off, dev_addr + 0x0, 4) < 0)
            user_panic("ide_read err 0x0\n");
        if (syscall_read_dev((u_int)(dst + offset), dev_addr + 0x4000, 0x200) < 0)
            user_panic("ide_read err 0x4000\n");
        if (syscall_read_dev((u_int)&read_value, dev_addr + 0x20, 1) < 0)
            user_panic("ide_read err 0x20\n");
        status = 0;
        if (syscall_write_dev((u_int)&status, dev_addr + 0x30, 1) < 0)
